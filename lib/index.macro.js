...
function extMacro(_ref) {
  ...
  var libPath;
  references.dev && references.dev.forEach(function (path) {
  ...
  if (references["default"][0]) {
    ...
    programPath.node.body.splice(afterImportIndex, 0, t.importDeclaration([t.importSpecifier(t.identifier(extensionFnName), t.identifier('extensionFn'))], t.stringLiteral("".concat(libPath || 'extensions.macro', "/lib/helpers"))), t.variableDeclaration('const', [t.variableDeclarator(t.identifier(extensionsObjName), getExtensionNode(extensions))]));
  }
}